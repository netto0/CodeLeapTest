...
export default function MainScreen() {
  ...
  return (
    <div className="w-[800px] min-h-screen h-fit bg-[#FFFFFF] self-center relative max-w-dvw sm:max-w-[800px] overflow-hidden">
      <h1 className="flex items-center justify-between text-2xl text-white font-bold bg-[#7695ec] px-8 py-6 select-none z-30">
        ...
        <LuMenu
          className={`sm:hidden text-3xl z-10 ${mobileMenu && "text-[#b6b6b6]"} transition-all`}
          onClick={() => setMobileMenu(!mobileMenu)}
        />
      </h1>
      <div className="flex flex-col p-4 gap-4 w-dvw sm:w-full relative">
        
        <div className={`bg-[#7695ec] absolute shadow-lg ${mobileMenu ? "right-0" : "-right-200"} top-0 sm:hidden flex w-2/3 h-10 rounded-xl justify-between items-center px-4 text-white z-0 transition-all`}>
          <span
            title={`@${loggedUser.name}`}
